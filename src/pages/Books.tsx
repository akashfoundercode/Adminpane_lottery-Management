...
import { Search, Filter, ArrowUpDown, Eye, BookOpen, AlertTriangle, CheckCircle2, XCircle, Lock, Send } from 'lucide-react';

export const Books: React.FC = () => {
  const { books, games, booksPagination, fetchAgentBooks, markBookAsSold, markBookAsUnsold, requestBookReopen } = useAgent();
  ...
  const [reopenBookId, setReopenBookId] = useState<string | null>(null);
  ...
  const handleReopenRequest = async (bookId: string) => {
    if (!reopenReason.trim()) return;
    setUpdatingBookId(bookId);
    try {
      await requestBookReopen(bookId, reopenReason.trim());
      setReopenBookId(null);
      setReopenReason('');
    } finally {
      setUpdatingBookId(null);
    }
                  ...
                  <tr key={book.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {book.status === 'Unsold by Admin' && book.reopenRequestStatus !== 'Pending' && (
                          <button onClick={() => setReopenBookId(book.id)} disabled={updatingBookId === book.id} className="px-2.5 py-1.5 text-[10px] font-bold text-danger-main bg-red-50 border border-red-200 hover:bg-red-100 rounded-lg disabled:opacity-50">Request Reopen</button>
                        )}
                        {book.reopenRequestStatus === 'Pending' && <span className="px-2.5 py-1.5 text-[10px] font-bold text-amber-700 bg-amber-50 rounded-lg">Reopen Pending</span>}
                      ...
                      {reopenBookId === book.id && (
                        <div className="mt-2 flex items-center gap-1">
                          <input value={reopenReason} onChange={event => setReopenReason(event.target.value)} placeholder="Reason for reopening" className="min-w-0 flex-1 px-2 py-1.5 text-[10px] border border-border-light rounded-lg" />
                          <button onClick={() => handleReopenRequest(book.id)} disabled={!reopenReason.trim() || updatingBookId === book.id} aria-label="Submit reopen request" className="p-1.5 text-white bg-brand-emerald rounded-lg disabled:opacity-50"><Send className="w-3 h-3" /></button>
                        </div>
                      )}
                    </td>
                  </tr>
          ...
          paginatedBooks.map((book) => (
            <div key={book.id} className="premium-card p-4 bg-white border border-border-light shadow-sm space-y-3">
              ...
              <div className="flex gap-2">
                ...
                {book.status === 'Unsold by Admin' && book.reopenRequestStatus !== 'Pending' && (
                  <button onClick={() => setReopenBookId(book.id)} disabled={updatingBookId === book.id} className="flex-1 py-2 bg-red-50 text-danger-main border border-red-200 text-xs font-semibold rounded-lg disabled:opacity-50">Request Reopen</button>
                )}
                {book.reopenRequestStatus === 'Pending' && <span className="flex-1 py-2 bg-amber-50 text-amber-700 text-xs font-semibold rounded-lg text-center">Reopen Pending</span>}
